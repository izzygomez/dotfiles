...
# Only tmux version >= 3.2 supports `absolute-centre` usage, as in
# `tmux set-option -g status-justify absolute-centre`. It was implemented in [1]
# & was first included in release 3.2 [2].
# [1] https://github.com/tmux/tmux/commit/3eb91efba160eff0b077a5fee902edb632f7fdca
# [2] https://github.com/tmux/tmux/releases
tmux_version=$(tmux -V | awk '{print $2}')
tmux_major_version=$(echo "$tmux_version" | cut -d. -f1)
tmux_minor_version=$(echo "$tmux_version" | cut -d. -f2 | tr -d -c 0-9)
major_required=3
minor_required=2
does_tmux_support_absolute_centre=0
if [ "$tmux_major_version" -gt "$major_required" ] ||
    [ "$tmux_major_version" -eq "$major_required" -a \
        "$tmux_minor_version" -ge "$minor_required" ]; then
    does_tmux_support_absolute_centre=1
fi
...
# macos_notification_count.sh {
	...
	# sqlite3 -list "/var/folders/12/3s2lvd5n20500l6716gyt2f80000gn/0//com.apple.notificationcenter/db/db" 'select * from app_info'
...
# mailcount.sh {
	...
	export TMUX_POWERLINE_SEG_MAILCOUNT_MAILDIR_INBOX="/Users/izzyg/.mail/inbox/new"
...
# tmux_session_info.sh {
	...
	export TMUX_POWERLINE_SEG_TMUX_SESSION_INFO_FORMAT="#S" # was previously "#S:#I.#P"
# }
